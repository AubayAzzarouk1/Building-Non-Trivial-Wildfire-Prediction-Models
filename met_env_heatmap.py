...
def getimg(date, factor):
    file = open("../Original Data/daily_dewpoint/{0}-{1}.csv".format(date.strftime("%Y-%m-%d"), factor), "r") ###READ THE DATA FILE### ###CHANGE TO ROOT DIRECTORY OF DESIRED MET./ENV. FACTOR. KEEP {} FOR THE DATE###
    ...
    mask = plt.imread('../Original Data/fill.png')[:,:,0] ###MASK CALIFORNIA'S LANDMASS AREA OVER THE INTERPOLATION TO REMOVE ANY POINTS OUTSIDE OF CALIFORNIA###
    ...
    plt.imsave('../Original Data/daily_dewpoint/img/{0}-{1}.png'.format(date.strftime("%Y-%m-%d"), factor), interpolate, cmap="gray", vmin=0, vmax=120) ###CHANGE TO ROOT DIRECTORY OF DESIRED MET./ENV. HEATMAPS. KEEP {0} FOR THE DATE AND {1} FOR THE FACTOR###
    
    file = open('../Original Data/daily_dewpoint/npy/{0}-{1}.npy'.format(date.strftime("%Y-%m-%d"), factor), 'wb')                                      ###CHANGE TO ROOT DIRECTORY OF DESIRED MET./ENV. HEATMAPS. KEEP {0} FOR THE DATE AND {1} FOR THE FACTOR###
    np.save(file, np.nan_to_num(interpolate))
    print(np.nan_to_num(interpolate))
    if display:
...
while cdate <= datetime.datetime(2018,1,1): ###SET TO LATEST DATE WITHIN DATA###
    #dates.append(cdate)
